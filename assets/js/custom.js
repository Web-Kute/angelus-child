  ...
  $(document).ready(function () {
    ...
    $("button.openBtn").on("click", function (event) {
      ...
      $.ajax({
        ...
        data: {
          // We pass php values differently! display php content
    ...
    const boxPartners = document.getElementById("box-partners");

    const handleClose = (e) => {
      if (e.target.id !== "partners" && e.target.id !== "close-bottom") {
        partners.classList.remove("active");
        boxPartners.classList.remove("active");
        setTimeout(() => {
    ...
    const toggleOpenPartners = (fadePartners) => {
      fadePartners();
      boxPartners.classList.toggle("active");
    };

    const toggleShowOpacity = () => {
      toggleOpenPartners(() => {
        partners.classList.toggle("active");
        setTimeout(() => {
    ...
    openBtn.addEventListener("click", toggleShowOpacity);
    closeBtn.addEventListener("click", toggleOpenPartners);
    ...
    roundLogo = () => {
      ...
      document
        ...
        .style.setProperty("--heightRoundLogo", `${logoSize}px`);
    };
    roundLogo();
    ...
    $("a.top, .smooth-scroll").on("click", function (e) {
      ...
      const $urlBase = new URL(
        "https://www.angelus-plongee.com/" + $urlSection
      );
    ...
    heightAdaptWindow = () => {
      ...
      if (windowWidth > 960) {
        let heightHomeSection =
          heightHomeBody -
          (heightTitle + heightMenu + heightSubTitle + heightCTA);
        $("#home").css("height", heightHomeSection);
      } else if (windowWidth < 960 && $("#mobile-menu").length) {
        let heightHomeSection =
          heightHomeBody -
          (heightTitle + heightMobileMenu + heightSubTitle + heightCTA);
        $("#home").css("height", heightHomeSection);
